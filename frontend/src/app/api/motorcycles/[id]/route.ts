...
export async function GET(req: NextRequest, { params }: P) {
  ...
  const sql = getDb();
  try {
    const rows = await sql`SELECT * FROM motorcycles WHERE id = ${id}`;
...
export async function DELETE(req: NextRequest, { params }: P) {
  ...
  const sql = getDb();
  try {
    const ex = await sql`SELECT id FROM motorcycles WHERE id = ${id}`;
    if (!ex[0]) return err("Motosiklet bulunamadı", 404);
    await sql`DELETE FROM motorcycles WHERE id = ${id}`;
