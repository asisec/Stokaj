...
export async function PUT(req: NextRequest, { params }: P) {
  ...
  try {
    ...
    const rows = await sql`
      UPDATE motorcycles SET brand=${body.brand??ex[0].brand}, model=${body.model??ex[0].model}, year=${body.year??ex[0].year}, color=${body.color??ex[0].color}, chassis_number=${body.chassis_number??ex[0].chassis_number}, engine_number=${body.engine_number??ex[0].engine_number}, status=${body.status??ex[0].status}, purchase_price=${body.purchase_price??ex[0].purchase_price}, sale_price=${body.sale_price??ex[0].sale_price}, notes=${body.notes??ex[0].notes}, updated_at=NOW() WHERE id=${params.id} RETURNING *`;
    return ok(rows[0]);
  } catch (e) { return err(String(e), 500); }
}
