...
export async function GET(req: NextRequest, { params }: P) {
  ...
  const sql = getDb();
  try {
    const rows = await sql`SELECT * FROM spare_parts WHERE id = ${targetId}`;
...
export async function DELETE(req: NextRequest, { params }: P) {
  ...
  const sql = getDb();
  try {
    const ex = await sql`SELECT id FROM spare_parts WHERE id = ${targetId}`;
    if (!ex[0]) return err("Yedek parça bulunamadı", 404);
    await sql`DELETE FROM spare_parts WHERE id = ${targetId}`;
