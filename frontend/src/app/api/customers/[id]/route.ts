...
export async function GET(req: NextRequest, { params }: P) {
  ...
  const sql = getDb();
  try {
    const customers = await sql`SELECT * FROM customers WHERE id = ${targetId}`;
...
export async function PUT(req: NextRequest, { params }: P) {
  ...
  try {
    const ex = await sql`SELECT * FROM customers WHERE id = ${targetId}`;
    if (!ex[0]) return err("Müşteri bulunamadı", 404);
    const idNum = body.identity_number ?? ex[0].identity_number;
    if (idNum && !/^[1-9]\d{10}$/.test(idNum)) return err("Geçersiz T.C. Kimlik Numarası");
    const rows = await sql`UPDATE customers SET first_name=${body.first_name??ex[0].first_name}, last_name=${body.last_name??ex[0].last_name}, phone=${body.phone??ex[0].phone}, email=${body.email??ex[0].email}, address=${body.address??ex[0].address}, identity_number=${idNum??null}, updated_at=NOW() WHERE id=${targetId} RETURNING *`;
    return ok(rows[0]);
  } catch (e) { return err(String(e), 500); }
}
...
export async function DELETE(req: NextRequest, { params }: P) {
  ...
  const sql = getDb();
  try {
    const ex = await sql`SELECT id FROM customers WHERE id = ${targetId}`;
    if (!ex[0]) return err("Müşteri bulunamadı", 404);
    await sql`DELETE FROM customers WHERE id = ${targetId}`;
